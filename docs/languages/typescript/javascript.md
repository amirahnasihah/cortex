...
JSDoc annotations that come before a declaration will be used to set the type of that declaration. For example:
 ...
 js ` /** @type {number} */ var x ; x = 0 ; // OK x = false ; // OK?! ` Try
...
To enable errors in your JavaScript files add: `// @ts-check` to the first line in your `.js` files to have TypeScript raise it as an error.
 ...
 js ` // @ts-check /** @type {number} */ var x ; x = 0 ; // OK x = false ; // Not OK Type 'boolean' is not assignable to type 'number'. 2322 Type 'boolean' is not assignable to type 'number'. ` Try
...
TypeScript may offer you errors which you disagree with, in those cases you can ignore errors on specific lines by adding `// @ts-ignore` or `// @ts-expect-error` on the preceding line.
 ...
 js ` // @ts-check /** @type {number} */ var x ; x = 0 ; // OK // @ts-expect-error x = false ; // Not OK ` Try
...
### Type Checking JavaScript Files
 ...
 Contributors to this page: OT 6 Last updated: Jun 15, 2026
...
You don’t even have to give a value if it will be initialized later:
 ...
 js ` class C { constructor () { /** @type {number | undefined} */ this . prop = undefined ; /** @type {number | undefined} */ this . count ; } } let c = new C (); c . prop = 0 ; // OK c . count = "string" ; Type 'string' is not assignable to type 'number'. 2322 Type 'string' is not assignable to type 'number'. ` Try
...
The property inference rules described above work exactly the same way.
 ...
 js ` function C () { this . constructorOnly = 0 ; this . constructorUnknown = undefined ; } C . prototype . method = function () { this . constructorOnly = false ; Type 'boolean' is not assignable to type 'number'. 2322 Type 'boolean' is not assignable to type 'number'. this . constructorUnknown = "plunkbat" ; // OK, the type is string | undefined }; ` Try
...
This can be used to nest classes, for example:
 ...
 js ` class C {} C . D = class {}; ` Try
 And, for pre-ES2015 code, it can be used to simulate static methods:

 js ` function Outer () { this . y = 2 ; } Outer . Inner = function () { this . yy = 2 ; }; Outer . Inner (); ` Try
 It can also be used to create simple namespaces:

 js ` var ns = {}; ns . C = class {}; ns . func = function () {}; ns ; ` Try
 Other variants are allowed as well:

 js ` // IIFE var ns = ( function ( n ) { return n || {}; })(); ns . CONST = 1 ; // defaulting to global var assign = assign || function () { // code goes here }; assign . extra = 1 ; ` Try
...
For instance:
 ...
 js ` var obj = { a : 1 }; obj . b = 2 ; // Allowed ` Try
...
Like other special JS checking behaviors, this behavior can be changed by specifying a JSDoc type for the variable. For example:
 ...
 js ` /** @type {{a: number}} */ var obj = { a : 1 }; obj . b = 2 ; Property 'b' does not exist on type '{ a: number; }'. 2339 Property 'b' does not exist on type '{ a: number; }'. ` Try
...
The only exception is for properties that have multiple initializers as described above.
 ...
 js ` function Foo ( i = null ) { if (! i ) i = 1 ; var j = undefined ; j = 2 ; this . l = []; } var foo = new Foo (); foo . l . .push(...items: any[]): number' >push ( foo . i ); foo . l . .push(...items: any[]): number' >push ( "end" ); ` Try
...
For instance:
 ...
 js ` function bar ( a , b ) { console . log ( a + " " + b ); } bar ( 1 ); // OK, second argument considered optional bar ( 1 , 2 ); bar ( 1 , 2 , 3 ); // Error, too many arguments Expected 0-2 arguments, but got 3. 2554 Expected 0-2 arguments, but got 3. ` Try
...
## Var-args parameter declaration inferred from use of `arguments`
 ...
 js ` /** @param {...number} args */ function sum ( /* numbers */ ) { var total = 0 ; for ( var i = 0 ; i < arguments . length ; i ++) { total += arguments [ i ]; } return total ; } ` Try
...
In a `.js` file, there is no legal way to specify these in the extends clause. By default the type arguments will be `any`:
 ...
 js ` import { Component } from "react" ; class MyComponent extends Component { render () { this . props . b ; // Allowed, since this.props is of type any } } `
 Use JSDoc `@augments` to specify the types explicitly. for instance:

 js ` import { Component } from "react" ; /** * @augments {Component<{a: number}, State>} */ class MyComponent extends Component { render () { this . props . b ; // Error: b does not exist on {a:number} } } `

### In JSDoc references

 An unspecified type argument in JSDoc defaults to any:

 js ` /** @type {Array} */ var x = []; x . .push(...items: any[]): number' >push ( 1 ); // OK x . .push(...items: any[]): number' >push ( "string" ); // OK, x is of type Array<any> /** @type {Array.<number>} */ var y = []; y . .push(...items: number[]): number' >push ( 1 ); // OK y . .push(...items: number[]): number' >push ( "string" ); // Error, string is not assignable to number ` Try
...
### In function calls
 ...
 js ` var p = new Promise (( resolve , reject ) => { reject (); }); p ; // Promise<any>; `
...
### JSDoc Reference
 ...
 The TypeScript docs are an open source project. Help us improve these pages by sending a Pull Request ❤
 Contributors to this page: HN OT RC NS MH 15+ Last updated: Jun 15, 2026
...
- Unsupported tags
 ...
 The meaning is usually the same, or a superset, of the meaning of the tag given at jsdoc.app .
...
You can use most JSDoc type syntax and any TypeScript syntax, from the most basic like `string` to the most advanced, like conditional types .
 ...
 js ` /** * @type {string} */ var s ; /** @type {Window} */ var win ; /** @type {PromiseLike<string>} */ var ' >promisedString ; // You can specify an HTML Element with DOM properties /** @type {HTMLElement} */ var myElement = document . (selectors: any): any (+4 overloads)' >querySelector ( selector ); element . dataset . myData = "" ; ` Try
 ...
 js ` /** @type {number[]} */ var ns ; /** @type {Array.<number>} */ var jsdoc ; /** @type {Array<number>} */ var nas ; ` Try
...
For example, an object with properties ‘a’ (string) and ‘b’ (number) uses the following syntax:
 ...
 js ` /** * A map-like object that maps arbitrary `string` properties to `number`s. * * @type {Object.<string, number>} */ var stringToNumber ; /** @type {Object.<number, object>} */ var arrayLike ; ` Try
...
This lets you cast types to other types by adding a `@type` tag before any parenthesized expression.
 ...
 js ` /** * @type {number | string} */ var numberOrString = Math . random () < 0.5 ? "hello" : 100 ; var typeAssertedNumber = /** @type {number} */ ( numberOrString ); ` Try
 You can even cast to `const` just like TypeScript:

 js ` let one = /** @type {const} */ ( 1 ); ` Try
...
This syntax is TypeScript-specific and differs from the JSDoc standard:
 ...
 js ` // @filename: types.d.ts export type Pet = { name : string , }; // @filename: main.js /** * @param {import("./types").Pet} p */ function walk ( p ) { console . log ( `Walking ${ p . name } ...` ); } ` Try
 import types can be used to get the type of a value from a module if you don’t know the type, or if it has a large type that is annoying to type:

 js ` /** * @type {typeof import("./accounts").userAccount} */ var x = any' >require ( "./accounts" ). userAccount ; ` Try
...
### `@import`
 ...
 js ` // @filename: dog.js export class Dog { woof () { console . log ( "Woof!" ); } } // @filename: main.js /** @import { Dog } from "./dog.js" */ const d = new Dog (); // error! ` Try
...
The parameter may also be declared optional by surrounding the name with square brackets:
 ...
 js ` /** * @return {PromiseLike<string>} */ function ' >ps () {} /** * @returns {{ a: string, b: number }} - May use &apos; @returns &apos; as well as &apos; @return &apos; */ function ab () {} ` Try
...
### Property Modifiers
 ...
 js ` // @ts-check class Car { constructor () { /** @private */ this . identifier = 100 ; } printIdentifier () { console . log ( this . identifier ); } } const c = new Car (); console . log ( c . identifier ); Property 'identifier' is private and only accessible within class 'Car'. 2341 Property 'identifier' is private and only accessible within class 'Car'. ` Try
...
### `@override`
 ...
 js ` export class C { m () { } } class D extends C { /** @override */ m () { } } ` Try
...
### `@extends`
 ...
 js ` /** * @template T * @extends {Set<T>} */ class ' >SortableSet extends Set { // ... } ` Try
...
### `@deprecated`
 ...
 js ` /** @deprecated */ const apiV1 = {}; const apiV2 = {}; apiV ; - api V1
- api V2
 ` Try
...
### `@see`
 ...
 ts ` type = {
 t: T;
}' >Box < ' >T > = { t : ' >T } /** @see Box for implementation details */ type = { [K in keyof T]: Box<T>; }' >Boxify < ' >T > = { [ K in keyof ' >T ]: = {
 t: T;
}' >Box < ' >T > }; ` Try
...
### `@link`
 ...
 ts ` type = {
 t: T;
}' >Box < ' >T > = { t : ' >T } /** @returns A { @link Box } containing the parameter. */ function (u: U): Box<U>' >box < (u: U): Box<U>' >U >( u : (u: U): Box<U>' >U ): = {
 t: T;
}' >Box < (u: U): Box<U>' >U > { return { t : u }; } ` Try
...
### `@author`
 ...
 ts ` /** * Welcome to awesome.ts * @author Ian Awesome <i.am.awesome@example.com> */ ` Try
...
```
 js ` var void;
}' >someObj = { /** * @param {string} param1 - JSDocs on property assignments work */ void' >x : function ( param1 ) {}, }; /** * As do jsdocs on variable assignments * @return {Window} */ let Window' >someFunc = function () {}; /** * And class methods * @param {string} greeting The greeting to use */ Foo . prototype . sayHi = ( greeting ) => console . log ( "Hi!" ); /** * And arrow function expressions * @param {number} x - A multiplier */ let number' >myArrow = ( x ) => x * x ; /** * Which means it works for function components in JSX too * @param {{a: string, b: number}} props - Some param */ var React.JSX.Element' >fc = ( props ) => < , HTMLDivElement>' >div > { props . a . charAt ( 0 ) } </ , HTMLDivElement>' >div > ; /** * A parameter can be a class constructor, using Google Closure syntax. * * @param {{new(...args: any[]): object}} C - The class to register */ function registerClass ( object' >C ) {} /** * @param {...string} p1 - A &apos;rest&apos; arg (array) of strings. (treated as &apos;any&apos;) */ function fn10 ( p1 ) {} /** * @param {...string} p1 - A &apos;rest&apos; arg (array) of strings. (treated as &apos;any&apos;) */ function fn9 ( p1 ) { return p1 . join (); } ` Try
```
...
### Creating .d.ts Files from .js files
 ...
 The TypeScript docs are an open source project. Help us improve these pages by sending a Pull Request ❤
 Contributors to this page: OT NS RG R RH 11+ Last updated: Jun 15, 2026
...
An alternative to using a TSConfig file is the CLI, this is the same behavior as a CLI command.
 ...
 sh ` npx -p typescript tsc src/**/*.js --declaration --allowJs --emitDeclarationOnly --outDir types `
...
### JSDoc Reference
 What JSDoc does TypeScript-powered JavaScript support?
 The TypeScript docs are an open source project. Help us improve these pages by sending a Pull Request ❤
 Contributors to this page: OT TM SG JB SW 3+ Last updated: Jun 15, 2026